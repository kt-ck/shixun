...
import { setWishlist } from "@/features/roleFeature/roleFeature";
        ...
        dispatch(
          setWishlist(
            w.map((item: SimpleProduct) => ({
              ...item,
              images: item.images.map((item) => process.env.BaseUrl + item),
            }))
          )
        );
      });
      ...
      <Header height={80} px="md">
        <Group position="apart" sx={{ height: "100%" }}>
          ...
          <Group className={classes.hiddenMobile}>
            <LoginButton btnTitle="Log in" btnVariant={undefined} close={closeDrawer}/>
          </Group>
